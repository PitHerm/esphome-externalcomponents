...
s5_gr3p15kww3_ns = cg.esphome_ns.namespace('s5_gr3p15kww3')
s5_gr3p15kww3Component = s5_gr3p15kww3_ns.class_('s5_gr3p15kww3Component', cg.PollingComponent)
...
CONF_T_INV = "temperature_inv"

CONFIG_SCHEMA = cv.Schema({
    cv.GenerateID(): cv.declare_id(s5_gr3p15kww3Component),
...
def to_code(config):
    ...
    if CONF_T_INV in config:
        conf = config[CONF_T_INV]
        sens = yield sensor.new_sensor(conf)
        cg.add(var.set_t_inv_sensor(sens))
